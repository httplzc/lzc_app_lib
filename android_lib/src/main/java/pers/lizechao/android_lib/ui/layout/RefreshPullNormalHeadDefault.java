...
public class RefreshPullNormalHeadDefault extends RefreshPullNormalView {
    ...
    public RefreshPullNormalHeadDefault(@NonNull Context context, boolean isTop) {
        super(context, isTop);
    }

    @Override
    protected View onCreateView(Context context, ViewGroup viewGroup) {
        View view = LayoutInflater.from(context).inflate(R.layout.refresh_view, viewGroup, false);
        initView(view);
        return view;
    }
    ...
    protected void onViewStateChange(PullStatus newStatus) {
        switch (newStatus) {
            ...
            case PULL_CANCEL:
                ...
                refreshText.setText(getResources().getString(R.string.refresh_view_pull_down));
                break;
            case PULL_REFRESH:
                reFreshImg.setRotation(180);
                refreshText.setText(R.string.refresh_view_release_refresh);
                break;
