...
public class PicCultActivity extends TitleBaseActivity {
    ...
    private void initMoveImg(MoveImage moveImage) {
        ...
        maxleft = 20 * ScreenData.density;
        maxright = ScreenData.widthPX - cult_padding;
        maxtop = (PicRealHeight - backHeight) / 2f;
        maxbottom = PicRealHeight - maxtop;

        moveImage.setMaxleft(maxleft);
        moveImage.setMaxright(maxright);

        moveImage.setMaxtop(maxtop);
        moveImage.setMaxbottom(maxbottom);
    ...
    private void PreDealPic() {
        ...
        if ((float) (bitmapWidth) / bitmapHeight - PicRealWidth / (float) PicRealHeight > 0) {
            //宽占满
            float bili = PicRealWidth / (float) bitmapWidth;
            if (bitmapHeight * bili < backHeight) {
                //放大至

                float bei = backHeight / (float) bitmapHeight;
                final Matrix matrix = new Matrix();
                matrix.postScale(bei * 1.01f, bei * 1.01f);
                matrix.postTranslate(maxleft - 0.1f, (PicRealHeight - backHeight) / 2f - 0.1f);
