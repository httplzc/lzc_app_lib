...
//        }
        ...
        for (Map.Entry<String, XiangceData> integerEntry : xiangceDatas.entrySet()) {
            xiangceDataList.add(integerEntry.getValue());
        }
    ...
    private void initPopupwindow(List<XiangceData> xiangceList) {
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                popupWindow.dismiss();
                if (position != 0) {
                    picImgGridAdapter.setUriList(GetPhoneImgManager.GetAllPicByParentId(getContentResolver(), xiangceListAdapter.getXiangceDataList().get(position).getId(), -1));
                } else {
